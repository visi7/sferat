...
### Gjetje shtesë nga instalimi i Sentry-t (jo e zbatuar ende)
**Problemi:** `npm install` zbuloi **4 dobësi sigurie "high severity"** te `next` (versioni aktual `16.0.8`) dhe `postcss`/`sharp` (varësi të tij) — përfshirë probleme DoS dhe XSS të njohura publikisht (GHSA). Detaje te `npm audit`.

**Pse s'e rregullova automatikisht:** `npm audit fix` mund të kërkojë ngritje versioni (madje edhe major) të Next.js-it, gjë që rrezikon të thyejë diçka pa testim të kujdesshëm — jo veprim për ta bërë "në kalim", si audituesit e tjerë.

**Kur ta rimarrim:** kalim i veçantë, i qëllimshëm, për të ngritur versionin e Next.js-it dhe për të testuar krejt platformën pas ngritjes — jo bashkë me punë tjetër.
