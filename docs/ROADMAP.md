...
- **Kontroll para-lançimi (siguri, ligj, SEO)** — kalim i gjerë, disa gjetje:
  ...
  - **Mbetën si shënime për ty (jashtë kodit)**: domain vetjak ende i pazënë (`sferat.app` referohet në `sitemap.ts`/`robots.txt` por s'dihet nëse është blerë), email SMTP i vërtetë për prodhim (shërbimi parazgjedhje i Supabase ka limite të ulëta dërgimi), dhe monitorim gabimesh (Sentry ose i ngjashëm) — asnjë prej tyre s'kërkon kod tani, kërkojnë vendim/blerje nga ti.
